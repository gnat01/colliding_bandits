...
# after the same explore mechanism, in the exploit phase --

# we choose the arm that maximizes mu_avg(j) + sqrt(2*log(t)/n_j) for all j, j being the arms, t the time step, mu_avg the current average return on that arm
...
dfl <- length(eps_loop) * n_arms_loop
