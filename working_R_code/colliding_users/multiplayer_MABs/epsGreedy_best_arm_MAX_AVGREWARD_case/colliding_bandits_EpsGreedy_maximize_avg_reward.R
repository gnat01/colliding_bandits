...
# MAXIMIZE the AVERAGE reward -- not TOTAL as done in earlier case
...
# a new matrix -- that tells us how many times a given arm was actually played 

  m <- matrix(rep(1.0e-10, n_users*n_arms), ncol = n_arms) # setting this to a small nonzero value to avoid any division-by-zero problems while fundamentally not changing anything
  ...
  for (explore_time in 1:explore_phase) {
    ...
    for (u in 1:n_users) {
      ...
      num_t[u,update_arm] <- num_t[u,update_arm] + 1
    }
...
# normalize the total reward for each user,arm pair by how many times the arm got played -- NEW approach

  perf_t <- perf_t / num_t
  #l <- list(perf_t = perf_t, chosen_arms = chosen_arms)
  l <- list(perf_t = perf_t, num_t)
  
  return(l)
}

epsgreedy_2 <- function(n_users, n_arms, tot_tries, perf_t, num_t, ba, eps) { # the performance for each user, arm pair
  # and each user's best arm
  
# before calculating avg rewards per arm here -- we multiply perf_t by num_t to get just total reward
# at the end, we will normalize this back to avg rewards to report
    # this is convenient for eps greedy as our best arm never changes in this approach.
...
# to return avg reward
  perf_t <- perf_t / num_t

  # done with exploration -- we should return the full perf tibble and the arm distributions at each step
  l <- list(perf_t = perf_t, arm_dist_t = arm_dist_t, num_t = num_t)
  return(l)
}

# what are the total rewards for each player after eps-greedy learning finishes?

total_user_rewards <- function(final_perf_t, n_arms, eps) {
  
  n_users <- nrow(final_perf_t)
  rewards <- apply(final_perf_t, 1, sum)
  avg_rewards <- apply(final_perf_t, 1, mean) #over all arms
  
  U <- data.frame(
    n_arms = rep(NA, n_users),
    eps = rep(NA, n_users),
    rewards = rep(NA, n_users),
    avg_rewards = rep(NA, n_users)
  )
  
  U$n_arms <- n_arms
  U$eps <- eps
  U$rewards <- rewards
  U$avg_rewards <- avg_rewards
...
final_user_rewards_t <- as_tibble(final_user_rewards)
...
for (n_arms in n_arms_loop) {
  ...
  # make the explore phase length dep on number of arms
  explore_phase <- 5 * n_arms
...
perf_t <- l11[[1]]
num_t <- l11[[2]] # how many times was each arm played in the explore phase for each user? that is here
...
l1 <- epsgreedy_2(n_users, n_arms, tot_tries, perf_t, num_t, ba, eps)

final_perf_t <- l1[[1]] #final average rewards
final_arm_dist_t <- l1[[2]]
final_num_t <- l1[[3]]

# User rewards : final

fur <- total_user_rewards(final_perf_t, n_arms, eps)

if (ctr == 1) {
  final_user_rewards_t <- rbind(fur)
} else {
  final_user_rewards_t <- rbind(final_user_rewards_t, fur)
}

# what are the best arms with their metrics?
...
# How about an unscaled variable n_arms / eps?

final_join2_t$scaled_1 <- final_join2_t$n_arms / final_join2_t$eps

final_join2_t$f_scaled_1 <- factor(final_join2_t$scaled_1)

final_join2_t$n_users <- n_users
final_join2_t$f_n_users <- factor(final_join2_t$n_users)
...
g2 <- ggplot(data = final_join2_t, aes(x = log(n), y = log(m), colour = factor(eps))) + geom_point() + 
  ggtitle("Numbers are n_arms / eps")
g2 <- g2 + facet_wrap(~ f_scaled_1, nrow = 8)
print(g2)


# save data
write_csv(final_join2_t, "/Users/girishnathan/work/code/R/colliding_bandits/epsGreedy_best_arm_MAX_AVGREWARD_case/101_players_best_arm_rewards_full_2_explore_dep_arms_max_avgreward.csv",
         col_names = T)

write_csv(final_user_rewards_t, "/Users/girishnathan/work/code/R/colliding_bandits/epsGreedy_best_arm_MAX_AVGREWARD_case/101_players_final_user_rewards_full_2_explore_dep_arms_max_avgreward.csv",
          col_names = T)

# save all plots

pdf("/Users/girishnathan/work/code/R/colliding_bandits/epsGreedy_best_arm_MAX_AVGREWARD_case/101_players_best_arm_rewards_plots_full_2_explore_dep_arms_max_avgreward.pdf")
g <- ggplot(data = final_join2_t, aes(x = log(n), y = log(m), colour = factor(n_arms))) + 
  geom_point() 
...
g2 <- ggplot(data = final_join2_t, aes(x = log(n), y = log(m), colour = factor(eps))) + geom_point() + ggtitle("Numbers are n_arms / eps")
g2 <- g2 + facet_wrap(~ f_scaled_1, nrow = 8)
print(g2)
